       ...
       <div className='pb-5'>
           <div className=' flex justify-between items-center'>
             ...
             <div className='bg-[#282142] rounded-full flex items-center gap-2 px-4 py-3 mt-2'>
              ...
              <input type="text" onChange={(e)=>setInput(e.target.value)} placeholder='Search user....' className='bg-transparent border-none outline-noe text-white text-xs placeholder-[#c8c8c8] flex-1'/>
             </div>
       </div>
           ...
           {filteredUsers.map((user , index)=>(
              <div onClick={()=>{setSelectedUser(user);setUnseenMessages(prev=>({...prev,[user._id]:0}))}}
               key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
                  <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-[35px] rounded-full aspect-[1/1]'/>
